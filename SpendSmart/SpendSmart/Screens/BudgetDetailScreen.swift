...
struct BudgetDetailScreen: View {
  ...
  private func saveTransaction() {
    let transaction = Transaction(
      title: transactionTitle, amount: transactionAmount ?? 0.0, quantity: transactionQuantity ?? 1)
  ...
  var body: some View {
    ...
    Form {
      ...
      Section("Transactions") {
        VStack {
          Text(
            "Spent: \(String(describing: budgetCategory.total.formatted(.currency(code: Locale.currencyCode))))"
          )
          .font(.headline)
          ...
          Text(
            "Remaining: \(String(describing: budgetCategory.remaining.formatted(.currency(code: Locale.currencyCode))))"
          )
          .font(.caption)
